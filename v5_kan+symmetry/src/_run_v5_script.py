...
if winner == "CH-KAN":
    ...
    # so the plotted curve is the effective multiplicative correction (exact physics = 1).
    uc = uc / uc[np.argmin(np.abs(xu))] if uc[np.argmin(np.abs(xu))] != 0 else uc
    axes[1].plot(xu, uc, lw=2, color="#4c78a8", label="learned u-correction (gauge-fixed)")
    axes[1].axhline(1.0, color="k", ls="--", lw=1, label="exact physics (1)")
